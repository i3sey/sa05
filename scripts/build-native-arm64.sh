...
output_dir=${NATIVE_OUTPUT_DIR:-$repo_root/app/src/main/jniLibs/arm64-v8a}
...
readonly cc="$toolchain/bin/aarch64-linux-android${ANDROID_API}-clang"
...
"$ndk/ndk-build" \
    ...
    NDK_DEBUG=0
tun2socks_output="$work_dir/shadowsocks-android/core/src/main/libs/arm64-v8a/libtun2socks.so"
if [[ ! -f "$tun2socks_output" ]]; then
    echo "tun2socks build produced no arm64 executable" >&2
    exit 1
fi
...
python3 "$repo_root/scripts/check-elf-16kb.py" "$staging"/*.so
...
echo "Installed validated arm64 libraries in $output_dir"
